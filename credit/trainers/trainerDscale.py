...
class Trainer(BaseTrainer):
    def __init__(self, model: torch.nn.Module, rank: int):
        ...
        logger.info("Downscaling model single-step training")
        ...
        for i in batch_group_generator:
            ...
            with autocast(enabled=amp):
                if 'x_surf_LR' in batch:
                    ...
                    x = concat_and_reshape(batch['x_LR'], batch['x_surf_LR']).to(self.device)
                else:
                    # no x_surf
                    x = reshape_only(batch['x_LR']).to(self.device)
                ...
                if 'x_forcing_static_HR' in batch:
                    ...
                    x_forcing_batch = batch['x_forcing_static_HR'].to(self.device).permute(0, 2, 1, 3, 4)
                ...
                if 'y_diag_HR' in batch:
                    ...
                    # (batch_num, time, var, lat, lon) --> (batch_num, var, time, lat, lon)
                    y_diag_batch = batch['y_diag_HR'].to(self.device).permute(0, 2, 1, 3, 4).float()
                ...
                # single step predict
                y_pred = self.model(x, x_time_encode)
