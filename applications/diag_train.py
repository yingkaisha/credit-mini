...
from credit.losses.diag_loss import DiagLoss
from credit.datasets.diag_singlestep import Diag_Dataset
...
def load_dataset_and_sampler(
    ...
    logging.info(f"Diag dataset loaded")
...
def main(rank, world_size, conf, backend, trial=False):
    ...
    if conf["data"]["scaler_type"] == "std-diag":
        ...
        param_train = copy.deepcopy(param)
        param_train['filenames'] = train_files
        param_train['filename_surface'] = train_list_surf_ds
        param_train['filename_dyn_forcing'] = train_list_dyn_forcing_ds
        param_train['filename_diagnostic'] = train_list_diag_ds
        param_train['history_len'] = conf["data"]["history_len"]
        param_train['forecast_len'] = conf["data"]["forecast_len"]
        ...
        param_valid = copy.deepcopy(param)
        param_valid['filenames'] = valid_files
        param_valid['filename_surface'] = valid_list_surf_ds
        param_valid['filename_dyn_forcing'] = valid_list_dyn_forcing_ds
        param_valid['filename_diagnostic'] = valid_list_diag_ds
        param_valid['history_len'] = conf["data"]["valid_history_len"]
        param_valid['forecast_len'] = conf["data"]["valid_forecast_len"]
    ...
    train_criterion = DiagLoss(conf)
    valid_criterion = DiagLoss(conf, validation=True)
...
if __name__ == "__main__":
    description = "Train a diagnostic var model"
