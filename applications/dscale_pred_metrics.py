...
from credit.datasets.dscale_singlestep import Dscale_Predict
from credit.transforms import Normalize_Dscale, ToTensor_Dscale
...
def predict(rank, world_size, conf, p):
    ...
    test_files_LR = [file for file in upper_files_LR if any(year in file for year in test_years)]
    ...
    test_list_surf_ds_LR = [file for file in list_surf_ds_LR if any(year in file for year in test_years)]
    ...
    param_HR['filenames'] = test_files
    param_HR['filename_surface'] = test_list_surf_ds
    param_HR['filename_dyn_forcing'] = test_list_dyn_forcing_ds
    param_HR['filename_diagnostic'] = test_list_diag_ds
    param_HR['history_len'] = conf["data"]["history_len"]
    param_HR['forecast_len'] = conf["data"]["forecast_len"]
    
    param_LR['filenames'] = test_files_LR
    param_LR['filename_surface'] = test_list_surf_ds_LR
    param_LR['history_len'] = conf["data"]['dscale_input']["history_len"]
    param_LR['forecast_len'] = conf["data"]['dscale_input']["forecast_len"]
        ...
        for k, batch in enumerate(data_loader):
            ...
            if forecast_hour == 1:
                if 'x_surf_LR' in batch:
                    ...
                    x = concat_and_reshape(batch['x_LR'], batch['x_surf_LR']).to(device)
                else:
                    # no x_surf
                    x = reshape_only(batch['x_LR']).to(device)
                ...
                if 'x_forcing_static_HR' in batch:
                    ...
                    x_forcing_batch = batch['x_forcing_static_HR'].to(device).permute(0, 2, 1, 3, 4)
                ...
                if 'y_surf_HR' in batch:
                    y = concat_and_reshape(batch['y_HR'], batch['y_surf_HR']).to(device)
                else:
                    y = reshape_only(batch['y_HR']).to(device)
                
                if 'y_diag_HR' in batch:
                
                    # (batch_num, time, var, lat, lon) --> (batch_num, var, time, lat, lon)
                    y_diag_batch = batch['y_diag_HR'].to(device).permute(0, 2, 1, 3, 4).float()
                ...
                # -------------------------------------------------------------------------------------- #
                # start prediction
                y_pred = model(x, x_time_encode) #, x_time_encode
